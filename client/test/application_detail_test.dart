...
void main() {
  testWidgets(
    'application detail renders job, timeline, cover letter and resume',
    (tester) async {
        ...
        routes: [
          ...
          GoRoute(
            ...
            builder: (context, state) =>
                const Scaffold(body: Text('หน้ารายละเอียดงาน')),
          ),
        ],
      );
  ...
  testWidgets(
    'application detail renders accepted timeline when application is accepted',
    (tester) async {
      tester.view.physicalSize = const Size(390, 844);
      tester.view.devicePixelRatio = 1;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      expect(
        find.text('ผ่านการคัดเลือก'),
        findsNWidgets(2),
      ); // StatusChip + Timeline step
      expect(
        find.text('ยินดีด้วย คุณผ่านการคัดเลือกสำหรับตำแหน่งนี้'),
        findsOneWidget,
      );
    },
  );
  ...
  testWidgets('application detail renders error and allows retry', (
    ...
    final repo = _FakeApplicationRepository(
      error: const AppException('ไม่พบใบสมัคร'),
    );
